...
const AuthPage: React.FC = observer(() => {
  ...
  const handleItmoIdClick = () => {
    // Заглушка: Войти с ITMO ID
  };

  const handleRegisterSubmit = async () => {
    if (password !== passwordConfirm || !name.trim() || !email.trim() || !password) return;
    try {
          ...
          {mode === 'login' ? (
            ...
            >
              ...
              <form onSubmit={handleLogin} className="flex flex-col gap-8">
                ...
                <div className='h-[1px] bg-gray-200 w-full relative'><div className='h-fit w-fit px-4 bg-white absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 text-sm text-gray-500'>Или</div></div>
                <Button
                  type="button"
                  typeStyle="secondary"
                  fullWidth
                        ...
                        {(['student', 'teacher'] as const).map((r) => (
                          ...
                          >
                            {r === 'student'
                              ...
                              : <BookOpen className={`size-4.5 ${role === r ? 'text-black' : 'text-gray-400'}`} />
                            }
