...
from app.checkers.plugins.common.formulas import check_formula_numbering
...
@rule(
    code="practice.formulas.numbering",
...
class FormulaNumberingRule(BaseRule):
    async def check(self, pdf: ParsedPDF, config: dict[str, Any]) -> list[RuleResult]:
        return await check_formula_numbering(pdf, config)
