...
_NON_WORD_RE = re.compile(r"[^a-zа-яё0-9]+", re.IGNORECASE)
...
def _normalize_for_search(text: str) -> str:
    ...
    normalized = _NON_WORD_RE.sub(" ", collapsed.lower())
...
class AppendixLabelingRule(BaseRule):
    async def check(self, pdf: ParsedPDF, config: dict[str, Any]) -> list[RuleResult]:
        ...
        for heading in pdf.headings:
            ...
            if heading_page:
                matched_block = next((b for b in heading_page.text_blocks if b.text.strip() == raw), None)
                if matched_block:
